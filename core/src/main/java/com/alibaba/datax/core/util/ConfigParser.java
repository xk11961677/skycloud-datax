...
public final class ConfigParser {
     ...
     */
    public static Configuration parse(final String jobPath,String param) {
        Configuration configuration = ConfigParser.parseJobConfig(jobPath,param);
     ...
     * @return
     */
    public static Configuration parseJobConfig(final String path,String param) {
        String jobContent = getJobContent(path);
        String jobContentBefore = jobContent;
        if(!StringUtils.isEmpty(param)) {
            jobContent = SpelUtil.replace(param, jobContent);
        }
        Configuration config = Configuration.from(jobContent);

        String logFilePath = config.getString(CoreConstant.LOG_FILE_PATH);

        LOG.info("设置日志文件路径：{}", logFilePath);

        EtlJobFileAppender.contextHolder.set(logFilePath);
        EtlJobLogger.log("解析之前 内容:{}" + jobContentBefore);
        EtlJobLogger.log("解析之后 内容:{}" + jobContent);
